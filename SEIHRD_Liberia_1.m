%% BMED 4813 BHI: SEIHRD MODEL V1(CYRUS) 
...
%% Solving the ODEs
tspan = tt;
f = @(t,x) [-(1/N)*(beta_IR*x(1)*x(3)+beta_ID*x(1)*x(4)+beta_HR*x(1)*x(5)+beta_HD*x(1)*x(6));
            (1/N)*(beta_IR*x(1)*x(3)+beta_ID*x(1)*x(4)+beta_HR*x(1)*x(5)+beta_HD*x(1)*x(6))-alpha*x(2);
            ...
            delta*x(6)];
[t,xa]=ode45(f,tspan, [S E I I H H R D]);
...
legend('Susceptibles','Exposed','Infectious','Hospitalized','Recovered','Deceased');
xlabel('Time (Days)','FontSize',20);
