...
for count = 1:I
    population(count).incubation = log(lognrnd(12.7,4.31)); %sets first x population to be sick
end
        ...
        for pos = 1:N %loops per sick
            if (xs(count) == ixs(pos))&&(ys(count)==iys(pos))&&population(count).incubation <1 %checks if positions overlap
                ...
                if T<pT %determines if disease passes
                    population(count).incubation = log(lognrnd(12.7,4.31)); %sets incubation
...
N    = 4570;          %fit is 3900, normal is 1500000
E    = 32;
I    = I_eqn_1(a,end)/2;
H    = 1.4;
R    = 2.4;
...
opts = optimset('MaxIter',100,'Display','off');
p_estimate = fminsearch(@(par)RMSE(tspan,I_real_2,par, S, E, I, H, R, F, D),[beta_IR beta_ID beta_HR beta_HD beta_F theta alpha e_1 e_2 k_2 k_1 pie roe delta gamma], opts);

N    = 4570;          %fit is 3900, normal is 1500000
beta_IR = p_estimate(1);         %fit is 0.230, normal is 0.160
beta_ID = p_estimate(2);         %fit is 0.230, normal is 0.160
beta_HR = p_estimate(3);
beta_HD = p_estimate(4);
beta_F  = p_estimate(5);
theta   = p_estimate(6);
alpha   = p_estimate(7);
e_1     = p_estimate(8);
...
tspan = linspace(max(tt_1),max(tt_2),(165-NUM(a)));     %172 for SL
...
beta_IR_est(a) = p_estimate(1);         %fit is 0.230, normal is 0.160
beta_ID_est(a) = p_estimate(2);         %fit is 0.230, normal is 0.160
beta_HR_est(a) = p_estimate(3);
beta_HD_est(a) = p_estimate(4);
beta_F_est(a)  = p_estimate(5);
theta_est(a)   = p_estimate(6);
alpha_est(a)   = p_estimate(7);
e_1_est(a)     = p_estimate(8);
e_2_est(a)     = p_estimate(9);
k_2_est(a)     = p_estimate(10);
k_1_est(a)     = p_estimate(11);
pie_est(a)     = p_estimate(12);
roe_est(a)     = p_estimate(13);
delta_est(a)   = p_estimate(14);
gamma_est(a)   = p_estimate(15);
...
AverageParams = [mean(beta_IR_est);mean(beta_ID_est);mean(beta_HR_est);mean(beta_HD_est);mean(beta_F_est);mean(theta_est);mean(alpha_est);mean(e_1_est);mean(e_2_est);mean(k_2_est);mean(k_1_est);mean(pie_est);mean(roe_est);mean(delta_est);mean(gamma_est)];
stdParams = [std(beta_IR_est);std(beta_ID_est);std(beta_HR_est);std(beta_HD_est);std(beta_F_est);std(theta_est);std(alpha_est);std(e_1_est);std(e_2_est);std(k_2_est);std(k_1_est);std(pie_est);std(roe_est);std(delta_est);std(gamma_est)];
%%
tt_3    = [tt_1 tt_2];
tt_4    = [tt_1 tspan];
tt_4    = tt_4(1:165); %172
...
for ii = 1:runs
    plot(tt_4,I_eqn_3_Fs(ii,:),'Color','[0.66,0.66,0.66]','LineWidth',10);    
end
