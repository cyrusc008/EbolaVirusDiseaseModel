...
for t = 1:tEnd %loops for time
    ...
    for count = 1:N
       ...
       for state = 1:N
           D = rand(1,1);
           if (D<pDU) && (population(count).incubation == 1)
               ixs(count) = -2;
           elseif (D>pDU) && (population(count).incubation == 1)
               rxs(count) = ixs(count); 
               rys(count) = iys(count); 
               ixs(count) = -3;
           end       
           if (D<pDH) && (population(count).incubation == 1)
               ixs(count) = -2;
           elseif (D>pDH) && (population(count).incubation == 1)
               rxs(count) = ixs(count); 
               rys(count) = iys(count); 
               ixs(count) = -3;
           end                
           H = rand(1,1);  
           if (H<pH) && (population(count).incubation == 15)
               ixs(count) = -4;
           end
           F = rand(1,1);
           if (F<pF) && (population(count).incubation == 1)
               ixs(count) = -5;
           end
       end
