...
for count = 1:I
    population(count).incubation = floor(rand(1,1)*19+2); %sets first x population to be sick
end
...
for t = 1:tEnd %loops for time
    ...
    for count = 1:N
       ...
       for state = 1:N
           D = rand(1,1);
           if (D>pD) && (population(count).incubation == 1)
               ixs(count) = -2;
           elseif (D<pD) && (population(count).incubation == 1)
               xs(count) = ixs(count); 
               ys(count) = ixs(count); 
               ixs(count) = -3;
           end
    ...
    pause(0.01); %waits for display sake
    sick = [sick ; sum(ixs>0)]; %counts number of sick
    dead = [dead ; sum(ixs == -2)];
    recovered = [recovered ; sum(ixs == -3)];
    healthy = [healthy ; N-sick(t)-dead(t)-recovered(t)]; %counts number of healthy
end

toc
clf
plot(1:tEnd,healthy,1:tEnd,sick,1:tEnd,dead,1:tEnd,recovered)
legend('Susceptible','Infected','Dead','Recovered')