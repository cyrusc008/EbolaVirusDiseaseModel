...
beta_IR = 0.160;         %fit is 0.230, normal is 0.160
beta_ID = 0.160;         %fit is 0.230, normal is 0.160
beta_HR = 0.062;
beta_HD = 0.062;
beta_F  = 0.489;
theta   = 0.45049;
...
N = linspace(1,1500000,15000);
tic

for i = 1:length(N)
        S(i) = N(i)-E;
        f = @(t,x) [-(1./N(i)).*(beta_IR.*x(1).*x(3)+beta_ID.*x(1).*x(4)+beta_HR.*x(1).*x(5)+beta_HD.*x(1).*x(6)+beta_F.*x(1).*x(8));
                    (1./N(i)).*(beta_IR.*x(1).*x(3)+beta_ID.*x(1).*x(4)+beta_HR.*x(1).*x(5)+beta_HD.*x(1).*x(6)+beta_F.*x(1).*x(8))-alpha.*x(2);
                    (1-theta).*alpha.*x(2)-(1-pie).*e_1.*x(3)-pie.*e_2.*x(3);
                    theta.*alpha.*x(2)-(1-pie).*k_1.*x(4)-pie.*k_2.*x(4);
                    pie.*e_2.*x(3)-roe.*x(5);
                    pie.*k_2.*x(4)-delta.*x(6);
                    ...
                    (1-pie).*k_1.*x(4)-gamma.*x(8);
                    gamma.*x(8)+delta.*x(6)];
        [t,xa]=ode45(f,tspan, [S(i) E 0 0 0 0 0 0 0]);
        ymtx(i,:,:) = xa;
end
...
for j = 1:length(N);
    ...
    RMSE(j) = (immse(I_real, transpose(I_eqn)))^0.5;
end 
