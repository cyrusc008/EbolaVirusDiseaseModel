...
for t = 1:tEnd %loops for time
    ...
    %pause(0.1); %waits for display sake
    sick = [sick ; sum(ixs>0)]; %counts number of sick
    dead = [dead ; sum(ixs == -2)];
    healthy = [healthy ; N-sick(t)-dead(t)]; %counts number of healthy
end

toc
clf
plot(1:tEnd,healthy,1:tEnd,sick,1:tEnd,dead)
legend('Healthy','Sick','Dead')