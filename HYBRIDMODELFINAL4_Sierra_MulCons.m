...
p_estimate = fminsearch(@(par)RMSE(tspan,I_real_2,par, S, E, I, H, R, F, D),[beta_IR beta_ID beta_HR beta_HD beta_F e_2 k_2 k_1 pie roe delta gamma], opts);
...
e_2_est(a)     = p_estimate(6);
k_2_est(a)     = p_estimate(7);
k_1_est(a)     = p_estimate(8);
pie_est(a)     = p_estimate(9);
roe_est(a)     = p_estimate(10);
delta_est(a)   = p_estimate(11);
gamma_est(a)   = p_estimate(12);
...
AverageParams = [mean(beta_IR_est);mean(beta_ID_est);mean(beta_HR_est);mean(beta_HD_est);mean(beta_F_est);mean(e_2_est);mean(k_2_est);mean(k_1_est);mean(pie_est);mean(roe_est);mean(delta_est);mean(gamma_est)];
stdParams = [std(beta_IR_est);std(beta_ID_est);std(beta_HR_est);std(beta_HD_est);std(beta_F_est);std(e_2_est);std(k_2_est);std(k_1_est);std(pie_est);std(roe_est);std(delta_est);std(gamma_est)];
%%
tt_3    = [tt_1 tt_2];
tt_4    = [tt_1 tspan];
tt_4    = tt_4(1:165); %172
