...
opts = optimset('MaxIter',100000000,'Display','off');
...
tspan = linspace(max(tt_1),max(tt_2),(168-NUM(1)));
...
sx = size(cumsum(xaa(1:NUM(1))));
sy = size(transpose(I_eqn_2(:,a)));
dims = max(sx(1),sy(1));
I_eqn_3_F(a,:) = [[cumsum(xaa(1:NUM(1)));zeros(abs([dims 0]-sx))],[transpose(I_eqn_2(:,a));zeros(abs([dims,0]-sy))]];
...
box on;
    ...
    plot(tt_4,I_eqn_3_Fs,'r','LineWidth',1.5);
    
%    plot(tt_1,I_eqn_1M,'w','LineWidth',1.5);
%    plot(tspan,I_eqn_2M,'w','LineWidth',1.5);
for ii = 1:runs
    plot(tt_4,I_eqn_3_Fs(ii,:),'r','LineWidth',1.5);    
